...
| **CORE10-STAGING-DOCTOR-OBS-001** | **BLOCKED / L-7 GATE FILED** | _Engineering + SRE + ops_ — L-6 partial capture (PR #121); [L-7 token refresh + API base approval gate](./ZORA_WALAT_L7_CORE10_TOKEN_REFRESH_API_BASE_APPROVAL_GATE_2026_05_30.md) — **not** token refresh/login/staging access in L-7; future L-8 phrase `APPROVE L-8 READ-ONLY STAGING OPERATOR TOKEN REFRESH AND API BASE URL VERIFY ONLY`; **TOKEN_EXPIRED** + health **404 HTML** unresolved; **CORE10-BLK-CAPTURE-001** **OPEN**; full snapshot **NOT** captured; doctor/obs **NOT VERIFIED**; mutation **NO**; production/pilot/real-money/market **NO-GO** |
