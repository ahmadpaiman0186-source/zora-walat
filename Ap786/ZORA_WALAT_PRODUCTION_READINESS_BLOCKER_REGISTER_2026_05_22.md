...
| **CORE10-STAGING-DOCTOR-OBS-001** | **BLOCKED / L-8 VERIFY FAIL** | _Engineering + SRE + ops_ — [L-8 evidence](./ZORA_WALAT_L8_CORE10_TOKEN_REFRESH_API_BASE_VERIFY_EVIDENCE_2026_05_30.md): token refresh **NOT COMPLETED** (login skipped — env creds); `staging-api-smoke` **FAIL** `route_missing_or_wrong_deployment` / **nextjs_frontend** on staging host; status-check **BLOCKED** `TOKEN_EXPIRED`; deploy guard local **PASS** but hosted API JSON **not** proven; **CORE10-BLK-CAPTURE-001** **OPEN**; full snapshot **NOT EXECUTED**; doctor/obs **NOT VERIFIED**; no token print; mutation **NO**; production/pilot/real-money/market **NO-GO** |
