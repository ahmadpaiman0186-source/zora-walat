# Zora-Walat — Reboot brief for ChatGPT, Cursor Agent, and senior engineers

**Purpose:** Single canonical handoff after memory reset. Read this file first, then follow links.  
**Coverage window:** 2026-03-28 → 2026-05-20  
**Last updated:** 2026-05-20  
**Canonical memory:** This file + `PROJECT_MEMORY_ZORA_WALAT_MASTER.md`  
**Sanitization rule:** All evidence is enum-only, boolean-only, suffix-only, count-only — never secrets, DATABASE_URL, tokens, JWTs, Stripe keys, password hashes, full Stripe IDs, PII, or raw webhooks.
...
| **Business purpose** | International mobile top-up (airtime/data/calling) for diaspora and travelers — Stripe-hosted checkout, operator fulfillment |
| **Current product scope** | **Phase 1:** USD Stripe Checkout → webhook-confirmed PAID → fulfillment queue → terminal fulfilled state. Staging proofs through **L-11**. Flutter airtime checkout + Next.js web top-up. Wallet/calling broader features partial. |
| **Critical business rule** | **No pay, no service** — fulfillment and provider spend only after server-confirmed payment (`PAYMENT_SUCCEEDED` / PAID). Client return URLs and UI state do not grant service. |
| **Critical engineering rule** | **Zero duplicate transaction / zero duplicate fulfillment** in verified scope — webhook idempotency, fulfillment gate, duplicate-safe counters; L-4/L-5 resend proof. L-13 duplicate *refund event* not yet executed. |

**Super-System target behavior:** Detect failures, classify errors, protect money path, fail closed, propose repairs, require human approval for mutations — never false PASS.
...
| `app/success`, `app/cancel` | Stripe return routes — **investor gap** (hardcoded English/dev copy) |
| `app/history` | Session-scoped order history |
| `components/topup/` | Checkout UI, Stripe Elements, CTA guards |
| `messages/{en,fa,ar,tr}.ts` | i18n — `en` complete; others partial scaffold |
...
| `lib/l10n/app_*.arb` | Localizations — some test/dev strings remain in wallet ARB |
...
```text
User selects plan → create checkout (PENDING)
→ Stripe Hosted Checkout
→ checkout.session.completed webhook → server marks PAID
→ canOrderProceedToFulfillment → queue/worker → FULFILLED
→ operator status-check: PAID_CONFIRMED true, fulfillment count 1
```

### Fulfillment flow

- BullMQ/Redis Phase 1 queue; idempotent producer/worker.  
- Gate: `server/src/lib/phase1FulfillmentPaymentGate.js` — `canOrderProceedToFulfillment`.

### Refund / incident flow

- Operator `l11-refund-execute` (approval phrase) OR Stripe `charge.refunded` webhook (slim mirror).  
- Post-payment incident → **REFUNDED**; L-11 proof: single refund, fulfillment count still **1**.

### Operator harness

- Modes: login, checkout, status-check, phase1-truth, L-11 preflight/refund/diagnose, credential-rotation-*, etc.  
- **Current login:** HTTP **401** `invalid_credentials` — harness blocked until rotation (execute not done).
...
| **DB** | Neon Postgres; staging branch name in governance doc: `staging-stripe-test-2026-05-12` (project id in Ap786 — no connection strings) |
| **ORM** | Prisma |
...
- Investor summary: `AP786_ALL_PASSES_INVESTOR_PROOF.md` — **not** production certification.
...
| Paid-before-fulfillment | **PASS** — `canOrderProceedToFulfillment` |
| Duplicate webhook replay | **PASS** — L-4/L-5; `StripeWebhookEvent` idempotency |
| Fulfillment idempotency | **PASS** (staging + tests) |
| Cancel / decline / expired (no fulfill) | **PASS** — L-9, L-8, L-10 |
| Refund mirror | **L-11 PASS** (single full refund, test mode) |
| **L-12** partial refund | **PENDING** — not implemented |
| **L-13** duplicate refund event | **NOT EXECUTED** — checklist only |

### Staging proof ladder (L-1…L-13)

| Level | Status | Notes |
|-------|--------|-------|
| L-1…L-7 | **PASS** (evidence) | Day 1 + webhook ordering/unmatched tests |
| L-8 decline | **PASS** | |
| L-9 cancel | **PASS** | |
| L-10 expired | **PASS** (automated); live expire optional |
| L-11 full refund | **PASS** | Once; suffix `…04pvq0dr78` in evidence |
...
- **Not** “bank-grade” or “guaranteed instant delivery” in customer copy policy.
...
| Topic | Status |
|-------|--------|
| **Operator auth** | **BLOCKED** — staging login **401**; P-2 historical PASS may be stale |
| **Credential rotation** | Plan + tooling on `main`; **dry-run BLOCKED** (missing gitignored local email); **execute NOT RUN** |
| **Secrets in git** | **PASS** — `npm run secrets:scan` in CI/Guard |
...
| **Evidence rule** | Enum/boolean/suffix/count only — see header of every Ap786 file |
...
| `ZW_INTELLIGENCE_VERDICT` | **WARN** (static profile — review categories; not a money incident) |
...
- `messages/fa|ar|tr`: partial — many keys still English via spread.  
- Flutter `walletTopUpHint` in `app_en.arb`: “testing your integration” wording.  
- Header nav “How it works” / “Support”: non-linked placeholders on web.
...
## 15. TIMELINE SUMMARY (2026-03-28 → 2026-05-20)

| Period | Engineering focus |
|--------|-------------------|
| Late Mar 2026 | MVP baseline, Prisma, auth/OTP |
| Apr 2026 | CI, payment contracts, Phase 1 webhook→queue, state machine |
| Early May 2026 | Vercel slim health/ready, operator harness, Ap786 Day 1 |
| Mid May 2026 | L-4–L-11 proofs, refund mirror, negative paths |
| 2026-05-19 | Frontend production copy + CTA safety |
| 2026-05-19–20 | zw-doctor, Super-System Guard, PR #21 merge, global audit pack |
...
zora_walat/
├── app/                 # Next.js pages
├── components/topup/    # Web checkout UI
├── messages/            # Web i18n
├── lib/                 # Flutter app + shared
├── server/              # API (Vercel root), prisma, tools, tests
├── Ap786/               # Sanitized evidence (THIS FOLDER)
└── .github/workflows/   # ci.yml, super-system-guard.yml
```
